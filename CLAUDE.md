...
`mood_entries` няма FK към `auth.users` по замисъл — псевдонимна е (само `user_session_id`; мъртвата `ip_hash` е махната 2026-08-09), не е свързана със самоличност.
...
2. ~~**16 KB page size — ще блокира Play Store.**~~ Решено 2026-08-09:
   `sentry-android` 7.14.0 → 7.22.5 (7.16.0+ подравнява) и явен
   `androidx.datastore:datastore-preferences:1.1.7` (Glance тегли 1.1.1
   транзитивно, 1.1.3+ подравнява). Проверено по ELF program headers на трите
   `.so` в билднатия APK — всички `PT_LOAD` са `align 0x4000`. На устройство
   диалогът още не е гледан; той е и за debuggable билдове по принцип, така че
   може да не изчезне напълно — важното за Play е подравняването.
3. **`.env` тук е с placeholder Supabase** (`https://placeholder.supabase.co`).
   ...
   не са през Supabase.
4. ~~**`ScrollToTop` покрива текст** на Home~~ — поправено 2026-08-09: бутонът
   висеше на `bottom-40` (160px, средата на съдържанието); сега е
   `calc(88px + safe-area)`, точно над таб бара (80px + inset). На десктоп без
   промяна. Възможно застъпване с CookieConsent (`bottom-20`) при първо
   посещение — банерът рисува върху бутона, приемливо.
...
~~**Още не са deploy-нати:** `submit-mood` и `send-weekly-digest`~~ — deploy-нати
2026-08-09 (`submit-mood` v4; `send-weekly-digest` v2, вече `verify_jwt: false`,
така че причината №2 от раздела за дайджеста е затворена — остават cron-ът и
GUC-ът, все още решение на потребителя).

**CI: никога не е бил зелен.** Проверено 2026-08-09 през GitHub API: последният
успешен run на `ci.yml` е №7 от 26 април; всичките ~712 след него са червени.
Текущата причина (поне от май): „Unit tests" стъпката не подаваше Supabase env, а
`src/lib/supabase.ts` хвърля при import — локално `.env` ги пълни, затова
313/313 минават на всяка машина, а runner-ът пада. Поправено в `ci.yml` с
hermetic placeholder стойности (unit тестовете мокват мрежата). Зад тази стъпка
може да чакат други изненади (април е падал на E2E) — гледай първия run след
поправката.
...
2:1 контраст (виж комита).

`html.light .text-\[\#64748b\] { color: #94a3b8 }` в `index.css` правеше приглушения
текст **по-светъл** върху по-светлия фон — **2.56:1** на `#eef2f8`, под AA, и оттам
идваха ~84-те слаби места (футър линкове, подписи под графики, етикети в Settings).
Потребителят одобри обръщането на 2026-08-09: правилото вече дава **`#475569`**
(~6.7:1). Проверено с Playwright — computed style на футър линк в светла тема е
`rgb(71, 85, 105)`; йерархията спрямо основния текст (`#0f172a`) се запазва.
Съседното правило `.text-\[\#94a3b8\] → #64748b` (4.2–4.8:1 според фона) не е
пипано — то не е било под 2:1 и смяната му е отделно решение.
...
**Поправено глобално за web 2026-08-09** — контра-правило в `index.css` веднага
след override блока: `html.light .text-white.bg-slate-900` (+ `.bg-slate-800`,
`.bg-black`) остава бял, само върху самия елемент, без наследници. Sweep-нато:
единственият плътен тъмен фон с `text-white` е чипът в Alerts. Цветните
запълвания (`bg-[#10b981]`, градиентите — 31 места) са отделен въпрос: там
override-ът дава тъмен текст върху цветно, четимо е, дизайнерско решение е.

### `mood_entries.ip_hash` — МАХНАТА 2026-08-09
Колоната беше декларирана „for rate limiting", но никой никога не я писа (87
реда, 0 със стойност при проверката). Потребителят избра да се махне, не да се
пише: попълването ѝ значеше събиране на нов личен идентификатор, който Privacy
страницата не споменава — реална цена за хипотетичен спам проблем. Миграцията е
`20260809000000_drop_mood_entries_ip_hash.sql`, приложена в продукция.

**Съзнателно приета последица:** дневният лимит „по едно на ден" виси изцяло на
`user_session_id` — UUID, който клиентът сам си генерира. Изчистен localStorage
дава нов глас. Mood е общностна функция на доверие; ако данните някога станат
важни, чистото решение без IP е гласуване само за влезли потребители.

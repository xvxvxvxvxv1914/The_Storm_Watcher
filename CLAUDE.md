...
Две функции, нарочно разделени, и **две копия** на геомагнитната част — второто е
inline в `send-kp-alerts/index.ts`. Движат се заедно, като Kp каскадата.

- **`calcAuroraVisibility(lat, lon, kp)`** — само геомагнитно. Казва „доколко си
  на място за тази буря". Ползва се от алармите и от сравнителните списъци с
  градове (Aurora, AuroraMap), където мащабиране по локален час би направило
  половината редове нула само заради часовата зона.
...
**Алармите нарочно НЕ се гейтват по тъмнина.** Те отговарят на „тече ли буря" —
известие в четири следобед е точно начинът да разбереш, че да излезеш след мръкване.
Гейт по текуща тъмнина би заглушил дневните бури изцяло.
...
**🔴 НАЙ-СПЕШНОТО: `send-kp-alerts` не е deploy-ната от 13.08.**

Кодът в git е **три поправки напред** от това, което cron-ът върти в момента:

1. `abs(gmlat)` — без нея цялото южно полукълбо връща 0% при всяко Kp, а и трите
   аларми гейтват на `visibility > 0`. **Потребител в Австралия, Нова Зеландия
   или южно Чили не получава никакво storm известие.**
2. Ръб на овала `66 − 2·Kp` вместо `67 − 5.3·Kp` — старият слага овала в
   тропиците при високо Kp, тоест праща аларми на хора, които няма как да видят
   нищо.
3. Хоризонтният обхват (9°, квадратичен спад) — без него няма видимост под ръба.

Deploy-ът иска Supabase сесия, каквато няма на Windows машината
(`SUPABASE_ACCESS_TOKEN` не е зададен, `~/.supabase/access-token` липсва,
`supabase login` е интерактивен и виси в неинтерактивна обвивка):

```powershell
$env:SUPABASE_ACCESS_TOKEN = "sbp_..."   # https://supabase.com/dashboard/account/tokens
npx supabase functions deploy send-kp-alerts --project-ref srzfoxlmhxyulrgkchjr
```

**Решение на потребителя, отворено от 13.08: гейтът бие прага.**
`visibility > 0` мълчаливо отменя потребителския Kp праг. След калибрацията
София е 0% до Kp 8 — значи потребител в София, поискал аларми при Kp 5, няма да
получи нито една. Верно по физика, изненадващо по поведение. Три пътя: гейтът да
пада под някакъв минимум (напр. 5%), да стане настройка („само когато е видимо от
мен"), или да отпадне и прагът да решава сам. Не е избрано.

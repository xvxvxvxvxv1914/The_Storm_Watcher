...
повтаряният бъг в тази кодова база — вече седем пъти, всеки път в различен сървиз
(седмият: Dashboard показваше Kp 0.0 „QUIET“, 0 km/s, Bz 0.0 и X-ray клас A при
паднала заявка — поправено 25.09, сега „—“):
...
### rtsw feed-овете носят няколко спътника (открито 2026-09-25)

`rtsw_wind_1m.json` и `rtsw_mag_1m.json` редуват в един масив ACE, IMAP и SOLAR1,
всеки ред със своя `source`; `active` е само на SOLAR1. **Активният не значи
жив:** последната му проба за вятър беше отпреди 8.5 часа, докато другите два
пращаха всяка минута — и „най-новата активна" слагаше 8.5-часова стойност под
LIVE. Bz алармата пък взимаше последните 15 реда от **всички** източници: 7 минути
ACE и IMAP на смени, записани като „15 минути устойчиво".

Правилото е в договора (`sampleSelection`, `freshMinutes: 10`) и е едно навсякъде:
активният, ако изостава с ≤10 мин от най-новата проба изобщо, иначе най-новата.
Прозорецът за Bz е от **един** спътник. TS: `latestRtswSample`
([src/utils/rtswSource.ts](src/utils/rtswSource.ts)) — никоя страница не избира
ред сама; Deno: `sustainedBzFromFeed` в `bz.ts`, който `bz.test.ts` връзва към TS
версията. Swift/Kotlin — само вятърът; **Swift не е компилиран към 25.09**.

### Edge функциите се разминаваха с git (открито 2026-09-25)

На 20.09 бяха деплойнати `send-kp-alerts` v13, `delete-account` v5 и нова
`submit-contact` от код, който **не беше в нито един клон**. За `send-kp-alerts`
разликата беше реферален план в алармите (`_shared/planAccess.ts`) — вкаран в git
на 25.09 и деплойнат оттам. `delete-account` и `submit-contact` **още не са в git**.
Преди deploy на функция: `get_edge_function` и diff срещу git — deploy от git
иначе тихо изтрива чуждата промяна.

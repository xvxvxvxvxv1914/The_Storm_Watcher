...
export default function ISSGlobe({ globeWidth, auroraData, issLat, issLng, userLat, userLng, theme, active = true }: Props) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  useEffect(() => {
    ...
    }).catch(err => logError('ISSGlobe texture load failed', err));
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        logError('ISSGlobe aurora overlay update failed', err);
      }
    }, 1000);
  ...
  // Camera follows ISS position
  useEffect(() => {
    if (!globeRef.current) return;
    if (issLat != null && issLng != null) {
      globeRef.current.pointOfView({ lat: issLat, lng: issLng, altitude: 2 }, 1000);
    }
  }, [issLat, issLng]);
  ...
  return (
    <ErrorBoundary fallback={
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-6 text-center">
        <Satellite className="w-10 h-10 text-[#f97316]/50" />
        <p className="text-[#94a3b8] text-sm">Globe unavailable</p>
      </div>
    }>
      <Globe
        ref={globeRef}
        ...
        globeImageUrl={globeMaterial ? undefined : '/textures/earth-night.jpg'}
